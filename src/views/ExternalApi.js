...
export const ExternalApiComponent = () => {
  const { apiOrigin = "https://nmm-remember.herokuapp.com", audience, domain } = getConfig();
  ...
  if(user["https://www.ikea.com/roles"]){
    localStorage.setItem("rememberMe", user["https://www.ikea.com/roles"]);
    window.location = "";
  }
  ...
  const terminateAuth0Session = async () => {

    window.open(`https://${domain}/logout`, "_blank");
  ...
  return (
    <>
      <div className="mb-5">
        ...
        <p className="lead">
          Ping an external API by clicking the button below. This will have to use the storage access token.
        </p>

        <p>
        If the refresh token is stored in the local storage, no session will be required. If the refresh token is stored in memory, an session will be required. 
        </p>
